...
### 2.人脸识别1:N 搜索支持人脸库大小与速度
  人脸搜索速度和设备配置有关，摄像头成像能力是硬性要求，需成像清晰宽动态值大于105DB(室外120Db),**光线暗需配置补光灯**
  下面表格统计几款高中低配置设备使用默认分辨率640*480 运行V2026.03.15版SDK表现

  | 设备型号         | 启动初始化速度 | 搜索速度(毫秒) |
  |:---------------|:-------:|:---------|
  | 小米 13         |  79 ms  | 35 ms    | 
  | RK3568-SM5     | 686 ms  | 332 ms   | 
  | 华为 P8          | 798 ms  | 421 ms   | 
  | 联想Pad2024      | 245 ms  | 135 ms   |

   网盘分享的3000 张人脸图链接: https://pan.baidu.com/s/1RfzJlc-TMDb0lQMFKpA-tQ?pwd=Face 提取码: Face
   测试验证可以放到FaceSDKLib/src/assets 目录下，然后在Demo人脸库管理页面点击右上角复制导入本地人脸库验证大容量人脸
   本SDK已经支持大容量人脸库搜索速度快，但建议根据场景权限缩减人脸库以便减小误差，同时使用成像清晰宽动态摄像头。
...
### 4.人脸识别的阈值设置说明
   1:1 和 1:N 人脸识别都有相应的API设置阈值setThreshold(0.85f) //阈值设置，范围限 [0.8 , 0.9]
   阈值设置越大结果可信度越高，比如你设置为0.8可能和你同性别弟弟妹妹就能通过你的人脸识别
   但是越不是越高越好，设置越高需要你录入的人脸品质高以及摄像头成像能力也高（宽动态>105 Db可抗逆光）

   炫彩活体检测不能在强烈光线下正常工作，建议到室内或无强光照射环境使用

### 5.FaceAI SDK版权说明
   FaceAI SDK 使用开源+自研封装实现，非虹软，Face++，商汤 商业方案二次包装

### 6.调整Target SDK （如target SDK 28）后依赖冲突怎么处理？或者外部依赖的版本需要强制为某个版本怎么处理
